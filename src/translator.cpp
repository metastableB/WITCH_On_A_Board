...
bool Translator::store(std::string input, DekatronStore* store) {
	...
	if(sign != '+' || sign != '-' || sign != '*') {
		logObject.log( LogLevel::ERROR,"translator.cpp", "Received object with undefined sign.\n");
		return false;
	}else if (input.length() != 9 || input.length() != 5){
		logObject.log(LogLevel::ERROR, "translator.cpp","Received undefined input length.\n");
		return false;
	}
	if(sign == '+' && input.length() == 9) {
		...
		logObject.log(LogLevel::ERROR, "translator.cpp","Received undefined sign -value pair.");
		return false;
	}
	// Control should not reach here
	logObject.log(LogLevel::ERROR, "translator.cpp","Undefined behavior.");
	return false;
}
